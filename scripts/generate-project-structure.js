...
function shouldExclude(name, isDirectory) {
  ...
  if (name.startsWith('.') && !['.env', '.gitignore', '.eslintrc', '.prettierrc'].includes(name)) {
    return true;
  }
  ...
  if (!isDirectory) {
    ...
    const isRelevantExt = RELEVANT_EXTENSIONS.some(relevant => 
    ...
    if (!isRelevantExt && !['README', 'CHANGELOG', 'LICENSE', 'CONTRIBUTING'].some(prefix => 
...
function isAlwaysRelevant(name) {
  return ALWAYS_RELEVANT_DIRS.some(relevant => 
    relevant.toLowerCase() === name.toLowerCase()
  );
}
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (isDirectory) {
        ...
        if (currentDepth < maxDepth - 1) {
          tree += generateTree(itemPath, prefix + nextPrefix, maxDepth, currentDepth + 1);
        }
      } else {
        tree += '\n';
      }
